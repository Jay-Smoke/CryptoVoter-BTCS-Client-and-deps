 ...
 *
 ***************************************************************************/
#ifndef CRYPTOVOTER_H
#define CRYPTOVOTER_H

#include "iquestionsdao.h"
#include "mainwindow.h"
#include <QProcess>
class CryptoVoter : QObject
{
    Q_OBJECT
public:
    /*!
    ...
    */
    ...
    /*!
     * \brief Connects signals between the gui, the data provider and the process who runs vanitygen
    */
    bool init();

    IQuestionDAO *getQDAO() const;
    void setQDAO(IQuestionDAO *value);
...
public slots:
    /*!
     * \brief Order the data provider to refresh all questions information
    */
    void searchData();

    /*!
     * \brief Prints the standar output from de qprocess with the vanitygen
    */
    void printOutput();

    /*!
     * \brief Prints the standar error from de qprocess with the vanitygen
    */
    void printError();

    /*!
     * \brief Receives and separates all the questions read by de DAO to create the signals complete, pending and active
    */
    void parseQuestions(QVector<Question> vecQuestions);

    void parseAnswers(QVector<Answer> vecAns);

signals:
    void refreshComplete(QVector<Question> vec);
    void refreshPending(QVector<Question> vec);
    void refreshActive(Question question);
