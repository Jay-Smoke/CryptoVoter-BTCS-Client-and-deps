...
#include "ui_instructionsform.h"

InstructionsForm::InstructionsForm(QWidget *parent, QString manualFile, QString automaticFile) :
    QWidget(parent),
    ui(new Ui::InstructionsForm)
{
    ...
    if (!manualFile.isEmpty()){
        ...
        QTextStream in(&file);
        this->ui->txtManual->setPlainText(in.readAll());
        this->ui->txtManual->setReadOnly(true);
    }

    if (!automaticFile.isEmpty()){

        QFile file(automaticFile);
        if (!file.open(QFile::ReadOnly | QFile::Text)) {
            QMessageBox::warning(this, tr("Application"),
                                 tr("Cannot read file %1:\n%2.")
                                 .arg(automaticFile)
        ...
        this->ui->txtAutomatic->setPlainText(in.readAll());
        this->ui->txtAutomatic->setReadOnly(true);
