...
class VoteLogRegister : public QObject
    {
    public:
        static VoteLogRegister* getInstancia();
        ~VoteLogRegister();

        bool openDB();
        bool deleteDB();
        QSqlError lastError();
        bool insertVote(VoteRegister vote);
        QVector<VoteRegister> getVoteRegisters();
        QMap<QString,double> getAnswersToQuid(QString quid); //Answers, votes

    private:
        QSqlDatabase db;
        QString dataFile;

        static VoteLogRegister *instancia;

        VoteLogRegister(QObject *parent = 0);
