...
{
    ui->setupUi(this);
    connect(this->ui->bSave,SIGNAL(clicked()),this,SLOT(saveConfig()));
    connect(this->ui->bSearch,SIGNAL(clicked()),this,SLOT(searchPath()));
...
void ConfigForm::load(){
    this->ui->txtPath->setText(Configuration::getInstancia()->getWalletPath());
    ui->spnNetFee->setValue(Configuration::getInstancia()->getNetFee());
}

void ConfigForm::saveConfig(){
    Configuration::getInstancia()->setWalletPath(this->ui->txtPath->text());
    Configuration::getInstancia()->setNetFee(ui->spnNetFee->value());
    this->close();
}

bool ConfigForm::searchPath(){

    QString directory = QFileDialog::getExistingDirectory(this, "Select default Bitcoin-sCrypt wallet directory.", QDir::currentPath());

    if(directory.isEmpty()){
        return false;
    }

    this->ui->txtPath->setText(directory);
    return true;
}
