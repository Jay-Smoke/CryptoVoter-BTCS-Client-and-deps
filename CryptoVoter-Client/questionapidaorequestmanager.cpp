...
QuestionApiDAORequestManager *QuestionApiDAORequestManager::instance = 0;

QuestionApiDAORequestManager::QuestionApiDAORequestManager(QObject *parent) :
    QObject(parent)
{

}

QuestionApiDAORequestManager *QuestionApiDAORequestManager::getInstance()
{
    if(instance == 0)
        instance = new QuestionApiDAORequestManager();
    return instance;
}

void QuestionApiDAORequestManager::executeRequest( QNetworkRequest request)
{

    aManager->get(request);
}

void QuestionApiDAORequestManager::setup()
{
    aManager = new QNetworkAccessManager(this);
    connect(aManager, SIGNAL(finished(QNetworkReply*)), this, SLOT(httpReply(QNetworkReply*)));
}


void QuestionApiDAORequestManager::httpReply(QNetworkReply *reply)
{
    emit getResponse(reply);
