...
int main(int argc, char *argv[]){
    ...
    VoteLogRegister::getInstancia()->openDB();

    QApplication::setOrganizationName("Bitcoin sCrypt");
    QApplication::setOrganizationDomain("bitcoinscrypt.org");
    QApplication::setApplicationName("CryptoVoter");

    QFile file("crypto.css");
    ...
    a.setWindowIcon(QIcon(":images/bitcoinBlue.png"));
    ...
    // Some time to show the splash
    QThread::sleep(4);

    // Shows termns and conditions if were not accepted
    if (!Configuration::getInstancia()->getEULAAcceptance()){
        EULAForm *eula= new EULAForm(0,"eula.txt",Configuration::getInstancia()->getEULAAcceptance());
        eula->show();
    }
